#!/usr/bin/env node
/**
 * Generates static Location × Role cross pages for programmatic SEO.
 *
 * Output:
 *   /locations/{location}/{role}.html — one page per location×role combo (min 5 jobs)
 *
 * Examples:
 *   /locations/san-francisco/engineering.html
 *   /locations/new-york/ml-ai.html
 *   /locations/remote/data.html
 *
 * Usage: node scripts/build-location-role-pages.js
 */
...
// ─── Location definitions (same as build-location-pages.js) ───
const LOCATIONS = {
    'san-francisco': {
        name: 'San Francisco Bay Area', emoji: '🌉',
    ...
    'boston': {
        name: 'Boston', emoji: '🎓',
        pattern: /boston|cambridge,?\s*ma/i,
        region: 'US East Coast',
    },
    'denver': {
        name: 'Denver', emoji: '🏔️',
        pattern: /denver|boulder/i,
        region: 'US Mountain',
    },
    'washington-dc': {
        name: 'Washington, D.C.', emoji: '🏛️',
        pattern: /washington.*d\.?c|arlington.*va|mclean|bethesda|reston|tysons|northern virginia/i,
        region: 'US East Coast',
    },
    'amsterdam': {
...
// ─── Shared CSS (same as location pages) ───
const sharedCSS = `
    <style>
        :root {
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .cl-section-title { font-family: var(--font-display); font-size: 28px; font-weight: 400; margin-bottom: 20px; letter-spacing: -0.02em; }
...
function companyCardHtml(slug, roleJobCount) {
    ...
    return `        <a href="/companies/${esc(slug)}" class="cl-company-card">
            <div class="cl-company-top">
                ...
                <div>
                    ...
                    <div class="cl-company-meta">${esc(co.size)} &middot; Glassdoor ${co.glassdoor}/5 &middot; ${roleJobCount} roles</div>
                </div>
            </div>
...
// ─── Role descriptions for SEO ───
const ROLE_DESCRIPTIONS = {
    'engineering': 'Software engineering, infrastructure, platform, DevOps, SRE, frontend, backend, and full-stack roles.',
    'ml-ai': 'Machine learning, AI research, deep learning, NLP, computer vision, and research engineering roles.',
    'data': 'Data science, data engineering, analytics, and business intelligence roles.',
    'design': 'Product design, UX design, visual design, and design systems roles.',
    'product': 'Product management, program management, and product strategy roles.',
    'marketing': 'Marketing, growth, content, SEO, communications, developer relations, and community roles.',
    'sales': 'Sales, account management, business development, partnerships, and customer success roles.',
    'finance': 'Finance, accounting, FP&A, and investor relations roles.',
    'hr-people': 'Recruiting, people operations, talent acquisition, and HR roles.',
    'legal': 'Legal, compliance, policy, and regulatory roles.',
    'support': 'Customer support, premium support, and help desk roles.',
    'operations': 'Operations, logistics, procurement, facilities, and workplace roles.',
};

const MIN_JOBS = 5;

// ─── Generate a location×role page ───
function generatePage(locSlug, roleSlug) {
    const loc = LOCATIONS[locSlug];
    const role = ROLES[roleSlug];
    if (!loc || !role) return null;

    const matchingJobs = enrichedJobs.filter(j =>
        loc.pattern.test(j.location) && j.role === roleSlug
    );

    if (matchingJobs.length < MIN_JOBS) return null;

    const companySet = new Set(matchingJobs.map(j => j.company));
    const matchingCompanies = [...companySet].filter(s => COMPANIES[s]);

    // Count jobs per company for display
    const jobsByCompany = {};
    matchingJobs.forEach(j => { jobsByCompany[j.company] = (jobsByCompany[j.company] || 0) + 1; });

    // Sort companies by job count descending
    matchingCompanies.sort((a, b) => (jobsByCompany[b] || 0) - (jobsByCompany[a] || 0));
    ...
    const otherRolesInLocation = Object.keys(ROLES).filter(r => {
        if (r === roleSlug) return false;
        return enrichedJobs.filter(j => loc.pattern.test(j.location) && j.role === r).length >= MIN_JOBS;
    });

    // Same role in other locations (for cross-links)
    const sameRoleOtherLocations = Object.keys(LOCATIONS).filter(l => {
        if (l === locSlug) return false;
        return enrichedJobs.filter(j => LOCATIONS[l].pattern.test(j.location) && j.role === roleSlug).length >= MIN_JOBS;
    });

    const searchParam = locSlug === 'remote' ? 'Remote' : locationName;
...
<section class="cl-hero">
    <div class="container">
        <div class="cl-hero-pill"><span class="cl-hero-pill-dot"></span>${esc(loc.region)} &middot; ${esc(roleName)}</div>
        <h1>${loc.emoji} <em>${esc(roleName)}</em> Jobs in ${esc(locationName)}</h1>
        <p class="cl-hero-sub">${esc(ROLE_DESCRIPTIONS[roleSlug] || '')} Browse open ${roleName.toLowerCase()} positions at top AI & tech companies hiring in ${locationName}.</p>
        <div class="cl-stats">
...
<section class="cl-companies">
    <div class="container">
        <h2 class="cl-section-title">Companies hiring <em>${esc(roleName.toLowerCase())}</em> in ${esc(locationName)}</h2>
        <div class="cl-company-grid">
${matchingCompanies.map(s => companyCardHtml(s, jobsByCompany[s] || 0)).filter(Boolean).join('\n')}
        </div>
    </div>
</section>
...
<section class="cl-jobs">
    <div class="container">
        <h2 class="cl-section-title">Latest ${esc(roleName.toLowerCase())} jobs in ${esc(locationName)}</h2>
        <div class="cl-job-list">
${showJobs.map(j => jobCardHtml(j)).join('\n')}
        </div>
        <a href="/jobs?search=${encodeURIComponent(searchParam)}&role=${encodeURIComponent(roleSlug)}" class="cl-show-more">View all ${matchingJobs.length.toLocaleString()} ${roleName.toLowerCase()} jobs &rarr;</a>
    </div>
</section>

<section class="cl-content">
    <div class="container">
        <h2>${esc(roleName)} jobs in ${esc(locationName)}</h2>
        <p>JobsByCulture tracks ${matchingJobs.length.toLocaleString()} open ${roleName.toLowerCase()} positions across ${matchingCompanies.length} companies in ${esc(locationName)}. Every company is rated on 18 culture values from real employee reviews, so you can find roles that match both your skills and how you like to work.</p>
        <p>See all <a href="/locations/${esc(locSlug)}" style="color:var(--accent);text-decoration:underline">${esc(locationName)} jobs</a> or browse all <a href="/roles/${esc(roleSlug)}" style="color:var(--accent);text-decoration:underline">${esc(roleName)} jobs</a> across all locations.</p>
    </div>
</section>`;

    // Cross-links: other roles in this location
    if (otherRolesInLocation.length > 0) {
        html += `

<section class="cl-related">
    <div class="container">
        <h2 class="cl-section-title">Other roles in ${esc(locationName)}</h2>
        <div class="cl-related-grid">
${otherRolesInLocation.map(r => {
    const rData = ROLES[r];
    const count = enrichedJobs.filter(j => loc.pattern.test(j.location) && j.role === r).length;
    return `            <a href="/locations/${esc(locSlug)}/${esc(r)}" class="cl-related-link">${esc(rData.ico)} ${esc(rData.name)} <span class="count">${count.toLocaleString()}</span></a>`;
}).join('\n')}
        </div>
    </div>
</section>`;
    ...
    // Cross-links: same role in other locations
    if (sameRoleOtherLocations.length > 0) {
        html += `

<section class="cl-related">
    <div class="container">
        <h2 class="cl-section-title">${esc(roleName)} jobs in other locations</h2>
        <div class="cl-related-grid">
${sameRoleOtherLocations.map(l => {
    const lData = LOCATIONS[l];
    const count = enrichedJobs.filter(j => lData.pattern.test(j.location) && j.role === roleSlug).length;
    return `            <a href="/locations/${esc(l)}/${esc(roleSlug)}" class="cl-related-link">${lData.emoji} ${esc(lData.name)} <span class="count">${count.toLocaleString()}</span></a>`;
}).join('\n')}
        </div>
    </div>
</section>`;
...
<script type="application/ld+json">
{
    ...
    return {
        ...
        roleSlug,
        html,
        path: `locations/${locSlug}/${roleSlug}.html`,
...
// ─── Main build ───
console.log('Building location × role pages...\n');

// Clean up old location subdirectories (but not the location .html files themselves)
const locDir = resolve(ROOT, 'locations');
if (existsSync(locDir)) {
    for (const entry of readdirSync(locDir, { withFileTypes: true })) {
        if (entry.isDirectory()) {
            rmSync(resolve(locDir, entry.name), { recursive: true, force: true });
        }
    }
}
...
for (const locSlug of Object.keys(LOCATIONS)) {
    const subDir = resolve(ROOT, 'locations', locSlug);
    let dirCreated = false;

    for (const roleSlug of Object.keys(ROLES)) {
        const result = generatePage(locSlug, roleSlug);
        if (result) {
            if (!dirCreated) {
            ...
            console.log(`  [loc×role] ${result.path} (${result.jobs} jobs, ${result.companies} companies)`);
        }
    }
}

console.log(`\nGenerated ${generated.length} location × role pages.`);

// ─── Update sitemap.xml ───
const sitemapPath = resolve(ROOT, 'sitemap.xml');
let sitemap = readFileSync(sitemapPath, 'utf-8');

// Remove old location×role entries
sitemap = sitemap.replace(/\s*<!-- Location×Role pages -->[\s\S]*?<!-- \/Location×Role pages -->\n?/g, '\n');
...
console.log(`Updated sitemap.xml with ${generated.length} location × role page URLs.`);
