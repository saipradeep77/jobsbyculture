    ...
    <tr>
      <td align="center">
        <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="background-color:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.06);">
          <!-- Top gradient bar -->
          <tr>
            <td style="height:5px;background:linear-gradient(90deg,#e8590c,#0d9488);font-size:0;line-height:0;">&nbsp;</td>
          </tr>
            ...
            <td style="padding:32px 40px 0 40px;">
              <table role="presentation" cellpadding="0" cellspacing="0">
                <tr>
                  ...
                  <td style="padding-left:10px;font-size:18px;font-weight:700;color:#52525b;">JobsByCulture</td>
                </tr>
              </table>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:32px 40px 16px 40px;">
              <p style="margin:0 0 8px 0;font-size:14px;font-weight:600;color:#e8590c;text-transform:uppercase;letter-spacing:0.5px;">Culture Card</p>
              <h1 style="margin:0 0 16px 0;font-size:28px;font-weight:700;color:#1a1a1f;line-height:1.25;">${escapeHtml(title)}</h1>
              <p style="margin:0 0 8px 0;font-size:16px;color:#52525b;line-height:1.5;">From <strong>${escapeHtml(teamName)}</strong></p>
...
export default async function handler(request) {
  ...
  try {
    ...
    if (!cardId || !cardSlug || !recipientEmail) {
      return new Response(JSON.stringify({ error: 'Missing required fields: cardId, cardSlug, recipientEmail' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json' },
      });
    }
    ...
    const resendRes = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        to: recipientEmail,
        subject: `\u{1F389} You received a Culture Card: ${card.title}`,
        html: buildEmailHtml({
          title: card.title,
